...
class tasmotaPlugin(octoprint.plugin.SettingsPlugin,
	...
	def turn_on(self, plugip, plugidx, username="admin", password="", backlog_delay=0):
		...
		try:
			if int(backlog_delay) > 0:
				webresponse = requests.get("http://" + plugip + "/cm?user=" + username + "&password=" + password + "&cmnd=backlog%20delay%20" + str(int(backlog_delay)*10) + "%3BPower" + str(plugidx) + "%20on%3B")
				response = dict()
				response["POWER%s" % plugidx] = "ON"
			else:
				webresponse = requests.get("http://" + plugip + "/cm?user=" + username + "&password=" + password + "&cmnd=Power" + str(plugidx) + "%20on")
	...
	def turn_off(self, plugip, plugidx, username="admin", password="", backlog_delay=0):
		...
		try:
			if int(backlog_delay) > 0:
				webresponse = requests.get("http://" + plugip + "/cm?user=" + username + "&password=" + password + "&cmnd=backlog%20delay%20" + str(int(backlog_delay)*10) + "%3BPower" + str(plugidx) + "%20off%3B")
				response = dict()
				response["POWER%s" % plugidx] = "OFF"
			else:
				webresponse = requests.get("http://" + plugip + "/cm?user=" + username + "&password=" + password + "&cmnd=Power" + str(plugidx) + "%20off")
				response = webresponse.json()
			chk = response["POWER%s" % plugidx]
		except:
			self._tasmota_logger.error('Invalid ip or unknown error connecting to %s.' % plugip, exc_info=True)
	...
	def check_status(self, plugip, plugidx, username="admin", password=""):
		...
		if plugip != "":
			try:
				webresponse = requests.get("http://" + plugip + "/cm?user=" + username + "&password=" + password + "&cmnd=Power" + str(plugidx))
